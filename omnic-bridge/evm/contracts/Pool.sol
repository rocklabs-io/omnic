...
contract Pool is LPTokenERC20, ReentrancyGuard {
    ...
    event RemoveLiquidity(address from, uint256 amountLP, uint256 amountSD, address to);
    event Swap(
        uint16  dstchainId,
        uint256 dstPoolId,
        address from,
    ...
    constructor(
        ...
        require(_unifiedDecimals <= _tokenOriginDecimals, "common decimals must be little more than token origin decimals");
    ...
    function addLiquidity(address _to, uint256 _amountLD) external nonReentrant onlyRouter returns (uint256) {
    ...
    function removeLiquidity(
        ...
        emit RemoveLiquidity(_from, _amountLP, amountSD, _to);
    }
    ...
    function _burn(address _from, uint256 _amountLP) internal returns (uint256 amountSD) {
